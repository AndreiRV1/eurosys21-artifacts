...
for rawf in rawfiles:
	...
	with open(join("rawdata/" + size, rawf)) as f:
		prev = 0
		for line in f:
			match_time = re.match(r"(\d+) trace_boot_time_init.+", line)
			match_plat = re.match(r"(\d+) trace_boot_plat_init.+", line)
			match_pcib = re.match(r"(\d+) trace_boot_ctor_beg: " + functions["pci-bus"] + r".+", line)
			match_pcie = re.match(r"(\d+) trace_boot_ctor_end: " + functions["pci-bus"] + r".+", line)
			match_viobb = re.match(r"(\d+) trace_boot_ctor_beg: " + functions["virtio-bus"] + r".+", line)
			match_viobe = re.match(r"(\d+) trace_boot_ctor_end: " + functions["virtio-bus"] + r".+", line)
			match_viopb = re.match(r"(\d+) trace_boot_ctor_beg: " + functions["virtio-pci"] + r".+", line)
			match_viope = re.match(r"(\d+) trace_boot_ctor_end: " + functions["virtio-pci"] + r".+", line)
			match_vionb = re.match(r"(\d+) trace_boot_ctor_beg: " + functions["virtio-net"] + r".+", line)
			match_vione = re.match(r"(\d+) trace_boot_ctor_end: " + functions["virtio-net"] + r".+", line)
			match_vfscorefb = re.match(r"(\d+) trace_boot_ctor_beg: " + functions["vfscore-fdtable"] + r".+", line)
			match_vfscorefe = re.match(r"(\d+) trace_boot_ctor_end: " + functions["vfscore-fdtable"] + r".+", line)
			match_vfscoreib = re.match(r"(\d+) trace_boot_ctor_beg: " + functions["vfscore-init"] + r".+", line)
			match_vfscoreie = re.match(r"(\d+) trace_boot_ctor_end: " + functions["vfscore-init"] + r".+", line)
			match_allocb = re.match(r"(\d+) trace_boot_alloc_beg.+", line)
			match_alloce = re.match(r"(\d+) trace_boot_alloc_end.+", line)
			match_ukbusb = re.match(r"(\d+) trace_boot_inittab_beg: " + functions["ukbus"] + r".+", line)
			match_ukbuse = re.match(r"(\d+) trace_boot_inittab_end: " + functions["ukbus"] + r".+", line)
			match_pthreadb = re.match(r"(\d+) trace_boot_inittab_beg: " + functions["pthread-embedded"] + r".+", line)
			match_pthreade = re.match(r"(\d+) trace_boot_inittab_end: " + functions["pthread-embedded"] + r".+", line)
			match_lwipb = re.match(r"(\d+) trace_boot_inittab_beg: " + functions["lwip"] + r".+", line)
			match_lwipe = re.match(r"(\d+) trace_boot_inittab_end: " + functions["lwip"] + r".+", line)
			match_rootfb = re.match(r"(\d+) trace_boot_inittab_beg: " + functions["vfscore-rootfs"] + r".+", line)
			match_rootfe = re.match(r"(\d+) trace_boot_inittab_end: " + functions["vfscore-rootfs"] + r".+", line)
			match_end = re.match(r"(\d+) trace_boot_end.+", line)
			if (match_time):
